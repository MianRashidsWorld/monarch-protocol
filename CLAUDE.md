...
Copy `.env.example` to `.env` and fill in real values before first run. Generate password hash with `make hash-password PASSWORD=yourpassword`.

```bash
# dev:    make dev          (docker compose up --build with hot-reload)
# migrate: make migrate     (apply Prisma migrations)
# seed:   make seed         (create Character #1 + starter rewards)
# studio: make studio       (Prisma Studio at localhost:5555)
# logs:   make logs         (tail app container logs)
# habits-check: make habits-check  (manually trigger HP deduction for missed habits)
```
...
```bash
# verify: docker compose exec app npx tsc --noEmit
```
...
**Stack:** Next.js 15 (App Router) + TypeScript. PostgreSQL via Prisma ORM. NextAuth v5 (Credentials provider). Tailwind CSS + shadcn/ui + Framer Motion. Containerized with Docker + docker-compose.

**Single-user app.** Character id=1 is always the one user. All queries use `characterId: 1` or `where: { id: 1 }` — never accept a character/user ID from the request.

**Key directories:**
- `src/actions/` — Next.js server actions (all mutations). Zod validates every input.
- `src/lib/game/` — Pure game logic functions (xp.ts, ranks.ts). No DB calls.
- `src/app/(app)/` — All protected routes (guarded by `middleware.ts`).
- `src/app/(auth)/` — Public auth routes.
- `src/app/api/cron/` — Cron endpoints, protected by `Authorization: Bearer <CRON_SECRET>`.

### Data flow rules

All mutations go through server actions in `src/actions/`. Pattern:
1. Zod parse at top of action (`safeParse`, return `{ error }` on failure).
2. DB writes in a `prisma.$transaction()` when multiple tables change.
3. Append a `QuestLog` entry for every XP/Gold/HP change — the log is the audit trail.
4. `revalidatePath()` after writes; pages use `export const dynamic = "force-dynamic"`.

Never call `process.env` directly — import from `src/lib/env.ts` (validated at startup).

### Safety rules for AI-assisted changes

- Never accept `characterId` from the client. Always hardcode `characterId: 1` in server code.
- Every new server action must start with a Zod schema and `safeParse`.
- New API routes under `src/app/api/` must have an explicit auth check at the top (session or `CRON_SECRET` header). No exceptions.
- QuestLog is append-only. Never update or delete log entries.
